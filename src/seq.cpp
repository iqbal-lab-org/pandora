...
void Seq::minimizer_sketch(const uint32_t w, const uint32_t k) {
    ...
    uint64_t shift1 = 2 * (k - 1), mask =
            (1ULL << 2 * k) - 1, smallest = std::numeric_limits<uint64_t>::max(), kmer[2] = {0, 0}, kh[2] = {0, 0};
    vector<Minimizer *> vm;
    ...
    for (uint32_t buff = 0; buff < seq.length(); ++buff) {
		...
		vm.pop_front();
            ...
            assert(vm.size() < w ||
                   assert_msg("we can't have added a smallest kmer correctly as vm still has size " << vm.size()));
