...
fs::path get_discovered_paths_fname(const GeneIntervalInfo &info,
    ...
    return fs::path(info.pnode->get_name() + "." + std::to_string(info.interval.start) + "-" +
                    std::to_string(info.interval.get_end()) + "_local_assembly_K" +
                    std::to_string(local_assembly_kmer_size) + ".fa");
}
...
void denovo_discovery::find_candidates(
    ...
    for (const auto &pileup: pileups) {
        ...
        const auto &info{pileup.first};
        const auto &interval_sequence{info.seq};

        const auto fname{get_discovered_paths_fname(info, local_assembly_kmer_size)};
        const auto denovo_paths_dir{output_directory / "denovo_paths"};
        fs::create_directories(denovo_paths_dir);
        const auto discovered_paths_fpath{denovo_paths_dir / fname};

        const auto read_covg{sequences.size()};
        const auto ref_length{interval_sequence.length()};
        const double expected_kmer_covg{
                denovo_discovery::calculate_kmer_coverage(read_covg, ref_length, local_assembly_kmer_size, error_rate)};

        BOOST_LOG_TRIVIAL(debug) << "Running local assembly for: " << info.pnode->get_name() << " - interval ["
                                 << info.interval.start << ", " << info.interval.get_end() << "]";

        local_assembly(sequences, interval_sequence, info.flank_seq_left, info.flank_seq_right, discovered_paths_fpath,
                       local_assembly_kmer_size, interval_sequence.length() * 2, expected_kmer_covg);
    }
}
