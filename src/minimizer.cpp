...
Minimizer::Minimizer(uint64_t s, uint32_t a, uint32_t b, bool c) : canonical_kmer_hash(s), pos_of_kmer_in_read(Interval(a, b)), strand(c) {
...
std::ostream &operator<<(std::ostream &out, Minimizer const &m) {
    out << "(" << m.canonical_kmer_hash << ", " << m.pos_of_kmer_in_read << ", " << m.strand << ")";
    return out;
}
