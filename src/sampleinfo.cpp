...
void SampleInfo::set_coverage_information(
...
std::vector<double> SampleInfo::get_likelihoods_for_all_alleles() const
{
    ...
    uint32_t total_mean_coverage_over_all_alleles_above_threshold
    ...
    for (size_t allele = 0; allele < get_number_of_alleles(); ++allele) {
        ...
        uint32_t total_mean_coverage_of_all_other_alleles_above_threshold
            = total_mean_coverage_over_all_alleles_above_threshold
            - total_mean_coverage_of_allele_above_threshold;

        double gaps = get_gaps(allele);

        double likelihood = compute_likelihood(min_coverage_threshold_is_satisfied,
            exp_depth_covg_for_this_sample,
            total_mean_coverage_of_allele_above_threshold,
            total_mean_coverage_of_all_other_alleles_above_threshold,
            genotyping_options->get_error_rate(), gaps);
...
uint32_t SampleInfo::get_total_mean_coverage_given_a_minimum_threshold(
...
uint32_t SampleInfo::get_total_mean_coverage_over_all_alleles_given_a_minimum_threshold(
    uint32_t minimum_threshold) const
{
    ...
    for (uint32_t allele = 0; allele < get_number_of_alleles(); ++allele)
        total_mean_coverage_over_all_alleles
            += get_total_mean_coverage_given_a_minimum_threshold(
                allele, minimum_threshold);
    return total_mean_coverage_over_all_alleles;
}

double SampleInfo::compute_likelihood(bool min_coverage_threshold_is_satisfied,
    double expected_depth_covg, double total_mean_coverage_of_allele_above_threshold,
...
boost::optional<SampleInfo::IndexAndConfidenceAndMaxLikelihood>
SampleInfo::get_confidence() const
{
    std::vector<double> likelihoods_for_all_alleles = get_likelihoods_for_all_alleles();

    size_t index_of_max_likelihood = Maths::arg_max(
        likelihoods_for_all_alleles.begin(), likelihoods_for_all_alleles.end());
    size_t index_of_second_max_likelihood; // guaranteed to exist, since we have at
                                           // least two alleles
    {
        std::vector<double> likelihoods_for_all_alleles_without_max_element(
            likelihoods_for_all_alleles.begin(), likelihoods_for_all_alleles.end());
        likelihoods_for_all_alleles_without_max_element.erase(
            likelihoods_for_all_alleles_without_max_element.begin()
            + index_of_max_likelihood);
    ...
    double second_max_likelihood
        = likelihoods_for_all_alleles[index_of_second_max_likelihood];
    double confidence = std::abs(max_likelihood - second_max_likelihood);

    uint32_t total_mean_coverage_of_max_likelihood_allele
    ...
    bool genotyping_from_compatible_coverage) const
{
    ...
    if (!only_one_flag_is_set) {
        fatal_error("Error on stringifying VCF record sample info: incompatible "
                    "genotyping options");
    }
    ...
    auto uint32_methods_to_call = { &SampleInfo::get_mean_forward_coverage,
        &SampleInfo::get_mean_reverse_coverage,
        &SampleInfo::get_median_forward_coverage,
        &SampleInfo::get_median_reverse_coverage, &SampleInfo::get_sum_forward_coverage,
        &SampleInfo::get_sum_reverse_coverage };
...
void SampleInfo::merge_other_sample_info_into_this(const SampleInfo& other)
{
    ...
    std::vector<std::vector<uint32_t>> allele_to_forward_coverages_merged(
        this->allele_to_forward_coverages);
    allele_to_forward_coverages_merged.insert(allele_to_forward_coverages_merged.end(),
        other.allele_to_forward_coverages.begin() + 1,
        other.allele_to_forward_coverages.end());

    std::vector<std::vector<uint32_t>> allele_to_reverse_coverages_merged(
        this->allele_to_reverse_coverages);
    allele_to_reverse_coverages_merged.insert(allele_to_reverse_coverages_merged.end(),
        other.allele_to_reverse_coverages.begin() + 1,
        other.allele_to_reverse_coverages.end());

    set_number_of_alleles_and_resize_coverage_information(
        this->get_number_of_alleles() + other.get_number_of_alleles() - 1);
    set_coverage_information(
        allele_to_forward_coverages_merged, allele_to_reverse_coverages_merged);
...
void SampleInfo::merge_other_sample_gt_from_max_likelihood_path_into_this(
    const SampleInfo& other, uint32_t allele_offset)
{
    ...
    if (not this->is_gt_from_max_likelihood_path_valid()) {
        if (other.get_gt_from_max_likelihood_path() == 0) {
            ...
            uint32_t new_gt_from_max_likelihood_path
                = other.get_gt_from_max_likelihood_path() + allele_offset - 1;
            this->set_gt_from_max_likelihood_path(new_gt_from_max_likelihood_path);
        }
    } else if (this->get_gt_from_max_likelihood_path() != 0
        or other.get_gt_from_max_likelihood_path() != 0) {
        // conflict, genotype using the coverages to solve
        boost::optional<SampleInfo::GenotypeAndMaxLikelihood>
            genotype_and_max_likelihood_from_coverage_optional
            = this->get_genotype_from_coverage();
        boost::optional<uint32_t> genotype_from_coverage = boost::none;
        if (genotype_and_max_likelihood_from_coverage_optional) {
            genotype_from_coverage
                = genotype_and_max_likelihood_from_coverage_optional->first;
        }
...
void SampleInfo::solve_incompatible_gt_conflict_with(SampleInfo& other)
{
    ...
    if (this->get_likelihood_of_gt_from_coverages_compatible()
        > other.get_likelihood_of_gt_from_coverages_compatible()) {
...
void SampleInfo::set_number_of_alleles_and_resize_coverage_information(
