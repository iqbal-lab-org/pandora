...
/*uint Read::find_position(const vector<uint16_t>& node_ids, const vector<bool>& node_orients)
{
    ...
    for (uint i=0; i<nodes.size(); ++i)
    {
        cout << "node " << i << " fwd " << nodes[i]->node_id << " " << node_orientations[i];
        cout << " and bwd " << nodes[nodes.size() -1 -i]->node_id << " ";
        cout << !node_orientations[node_orientations.size() -1  -i] << endl;

        if (nodes[i]->node_id == node_ids[0] and node_orientations[i] == node_orients[0])
        {
            cout << "start with node " << nodes[i]->node_id;
            cout << " which matched the first node of search pattern " << *node_ids.begin() << endl;
            search_pos = 0;
            found_pos = 0;
            while (i + found_pos < nodes.size()
                   and nodes[i + found_pos]->node_id == node_ids[search_pos]
        ...
        // if we are considering matches which overlap the end, also consider ones which overlap
        // the start by the same amount
        if (i + node_ids.size() >= nodes.size()
            and nodes[0] == node_ids[nodes.size() - i]
            and node_orientations[0] == node_orients[nodes.size() - i] )
        {
            search_pos = nodes.size() - i;
            found_pos = 0;
            while (found_pos < nodes.size()
                   and nodes[found_pos]->node_id == node_ids[search_pos]
                   and node_orientations[found_pos] == node_orients[search_pos]) {
        ...
        if (nodes[nodes.size() -1 -i]->node_id == node_ids.back()
                   and node_orientations[node_orientations.size() -1  -i] == !node_orients.back())
        {
            cout << "start with node " << nodes[nodes.size() -1 -i]->node_id;
            cout << " which matched the first node of search pattern in reverse " << node_ids.back() << endl;
            search_pos = 0;
            found_pos = 0;
            while (i + 1 < nodes.size() + found_pos
                   and nodes[nodes.size() -1 -i +found_pos]->node_id == node_ids[node_ids.size() -1 -search_pos]
                   and node_orientations[nodes.size() -1  -i +found_pos] == !node_orients[node_orients.size() -1 -search_pos])
            {
                cout << "bwd " << search_pos << " " << nodes.size() -1 -i +found_pos << endl;
