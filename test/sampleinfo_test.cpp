...
TEST(SampleInfoTest, constructor___one_allele)
{
    ...
    EXPECT_ITERABLE_EQ(std::vector<std::vector<uint32_t>>,
        sample_info.get_allele_to_forward_coverages(), default_coverages);
    EXPECT_ITERABLE_EQ(std::vector<std::vector<uint32_t>>,
        sample_info.get_allele_to_reverse_coverages(), default_coverages);
    EXPECT_EQ(sample_info.get_genotyping_options(), &default_genotyping_options);
    EXPECT_EQ(sample_info.get_exp_depth_covg_for_this_sample(),
        default_genotyping_options.get_sample_index_to_exp_depth_covg()[1]);
}
...
TEST(SampleInfoTest, constructor___five_alleles)
{
    ...
    std::vector<std::vector<uint32_t>> default_coverages { { 0 }, { 0 }, { 0 }, { 0 },
        { 0 } };
    EXPECT_ITERABLE_EQ(std::vector<std::vector<uint32_t>>,
        sample_info.get_allele_to_forward_coverages(), default_coverages);
    EXPECT_ITERABLE_EQ(std::vector<std::vector<uint32_t>>,
    ...
    EXPECT_EQ(sample_info.get_genotyping_options(), &default_genotyping_options);
    EXPECT_EQ(sample_info.get_exp_depth_covg_for_this_sample(),
        default_genotyping_options.get_sample_index_to_exp_depth_covg()[3]);
}
...
public:
    ...
    void SetUp() override
    {
        allele_to_coverage_one_allele
            = std::vector<std::vector<uint32_t>>({ { 1, 2 } });

        allele_to_coverage_two_alleles
            = std::vector<std::vector<uint32_t>>({ { 1, 2 }, { 3, 4 } });

        allele_to_coverage_three_alleles
            = std::vector<std::vector<uint32_t>>({ { 1, 2 }, { 3, 4 }, { 5, 6 } });
    }
...
TEST_F(
    SampleInfoTest___Fixture, get_and_set_gt_from_max_likelihood_path___set_a_valid_gt)
{
...
TEST_F(SampleInfoTest___Fixture,
    is_gt_from_max_likelihood_path_valid___default_sample_info___is_invalid)
{
    EXPECT_FALSE(default_sample_info.is_gt_from_max_likelihood_path_valid());
}

TEST_F(SampleInfoTest___Fixture,
    is_gt_from_max_likelihood_path_valid___set_valid_gt___is_valid)
{
    default_sample_info.set_gt_from_max_likelihood_path(5);
...
TEST_F(SampleInfoTest___Fixture,
    is_gt_from_max_likelihood_path_valid___set_valid_then_invalid_gt___is_invalid)
{
    default_sample_info.set_gt_from_max_likelihood_path(5);
    default_sample_info.set_gt_from_max_likelihood_path(boost::none);
    EXPECT_FALSE(default_sample_info.is_gt_from_max_likelihood_path_valid());
...
TEST_F(SampleInfoTest___Fixture,
    get_gt_from_max_likelihood_path___default_sample_info___throws_exception)
{
    try {
        default_sample_info.get_gt_from_max_likelihood_path();
        FAIL() << "Should have thrown exception";
...
TEST_F(SampleInfoTest___Fixture, get_allele_to_forward_coverages___default_sample_info)
{
    std::vector<std::vector<uint32_t>> actual
        = default_sample_info.get_allele_to_forward_coverages();
...
{
    std::vector<std::vector<uint32_t>> actual
        = default_sample_info.get_allele_to_reverse_coverages();
    std::vector<std::vector<uint32_t>> expected { { 0 }, { 0 } };
    ...
    set_coverage_information___forward_coverage_has_no_alleles___expects_FatalRuntimeError)
{
    ASSERT_EXCEPTION(default_sample_info.set_coverage_information(
        ...
        FatalRuntimeError,
        "Error when setting coverage information for sample: coverage information left "
        "inconsistent");
}
...
{
    ASSERT_EXCEPTION(
        default_sample_info.set_coverage_information(
...
TEST_F(SampleInfoTest___Fixture,
    set_coverage_information___both_coverages_have_two_alleles)
{
    default_sample_info.set_coverage_information(
        allele_to_coverage_two_alleles, allele_to_coverage_two_alleles);
...
TEST_F(SampleInfoTest___Fixture,
    set_coverage_information___both_coverages_have_three_alleles)
{
    default_sample_info_three_alleles.set_coverage_information(
    ...
    set_number_of_alleles_and_resize_coverage_information___resize_to_zero_alleles___expects_FatalRuntimeError)
{
    ...
    ASSERT_EXCEPTION(default_sample_info_three_alleles
        ...
        FatalRuntimeError,
        "Error when setting number of alleles for sample: coverage information left "
        "inconsistent");
}

TEST_F(SampleInfoTest___Fixture,
    set_number_of_alleles_and_resize_coverage_information___resize_to_one_allele___shrinks)
{
    default_sample_info_three_alleles.set_coverage_information(
        allele_to_coverage_three_alleles, allele_to_coverage_three_alleles);

    default_sample_info_three_alleles
        .set_number_of_alleles_and_resize_coverage_information(1);

    EXPECT_EQ(1, default_sample_info_three_alleles.get_number_of_alleles());
    EXPECT_EQ(allele_to_coverage_one_allele,
        default_sample_info_three_alleles.get_allele_to_forward_coverages());
    EXPECT_EQ(allele_to_coverage_one_allele,
        default_sample_info_three_alleles.get_allele_to_reverse_coverages());
}

TEST_F(SampleInfoTest___Fixture,
    set_number_of_alleles_and_resize_coverage_information___resize_to_two_alleles___shrinks)
{
    default_sample_info_three_alleles.set_coverage_information(
        allele_to_coverage_three_alleles, allele_to_coverage_three_alleles);

    default_sample_info_three_alleles
        .set_number_of_alleles_and_resize_coverage_information(2);

    EXPECT_EQ(2, default_sample_info_three_alleles.get_number_of_alleles());
    EXPECT_EQ(allele_to_coverage_two_alleles,
        default_sample_info_three_alleles.get_allele_to_forward_coverages());
    EXPECT_EQ(allele_to_coverage_two_alleles,
        default_sample_info_three_alleles.get_allele_to_reverse_coverages());
}
...
TEST_F(SampleInfoTest___Fixture,
    set_number_of_alleles_and_resize_coverage_information___resize_to_three_alleles___no_change)
{
    default_sample_info_three_alleles.set_coverage_information(
        allele_to_coverage_three_alleles, allele_to_coverage_three_alleles);

    default_sample_info_three_alleles
        .set_number_of_alleles_and_resize_coverage_information(3);

    EXPECT_EQ(3, default_sample_info_three_alleles.get_number_of_alleles());
    EXPECT_EQ(allele_to_coverage_three_alleles,
        default_sample_info_three_alleles.get_allele_to_forward_coverages());
    EXPECT_EQ(allele_to_coverage_three_alleles,
        default_sample_info_three_alleles.get_allele_to_reverse_coverages());
}
...
TEST_F(SampleInfoTest___Fixture,
    set_number_of_alleles_and_resize_coverage_information___resize_to_four_alleles___expands)
{
    default_sample_info_three_alleles.set_coverage_information(
        allele_to_coverage_three_alleles, allele_to_coverage_three_alleles);

    default_sample_info_three_alleles
        .set_number_of_alleles_and_resize_coverage_information(4);
    ...
    EXPECT_EQ(allele_to_coverage_four_alleles,
        default_sample_info_three_alleles.get_allele_to_forward_coverages());
    EXPECT_EQ(allele_to_coverage_four_alleles,
        default_sample_info_three_alleles.get_allele_to_reverse_coverages());
}
...
TEST_F(SampleInfoTest___Fixture,
    gt_coverages_compatible___default_sample_info___invalid_gt)
{
...
TEST_F(
    SampleInfoTest___Fixture, gt_coverages_compatible___default_sample_info___valid_gt)
{
...
TEST_F(SampleInfoTest___genotype_from_coverage___Fixture, valid_genotype)
{
    ...
    EXPECT_NEAR(
        -1.0, default_sample_info.get_likelihood_of_gt_from_coverages(), 0.000001);
...
private:
    ...
    public:
        using SampleInfo::SampleInfo;
        MOCK_METHOD(boost::optional<SampleInfo::GenotypeAndMaxLikelihood>,
            get_genotype_from_coverage, (), (const override));
    };
...
public:
    ...
    void SetUp() override
    {
        ...
        sample_info_with_two_alleles.set_coverage_information(
            allele_to_coverage_two_alleles, allele_to_coverage_two_alleles);
        ...
        sample_info_with_three_alleles.set_coverage_information(
            allele_to_coverage_three_alleles, allele_to_coverage_three_alleles);
    }
    ...
    merge_a_sample_with_three_alleles_into_one_with_two_alleles___same_gt_from_max_likelihood_path)
{
    ...
    sample_info_with_two_alleles.merge_other_sample_info_into_this(
        sample_info_with_three_alleles);

    std::vector<std::vector<uint32_t>> allele_to_coverage_expected(
        { { 1, 2 }, { 3, 4 }, { 5, 6 }, { 7, 8 } });
    EXPECT_EQ(allele_to_coverage_expected,
        sample_info_with_two_alleles.get_allele_to_forward_coverages());
    EXPECT_EQ(allele_to_coverage_expected,
        sample_info_with_two_alleles.get_allele_to_reverse_coverages());
...
public:
    ...
    SampleInfoTest___get_gaps___Fixture()
        : sample_info_with_min_kmer_covg_10(
            0, 2, &genotyping_options_with_min_kmer_covg_10)
    {
    }
...
{
    sample_info_with_min_kmer_covg_10.set_coverage_information(
        { { 5 }, { 5, 5, 5, 5 } }, { { 4 }, { 4, 4, 4, 4 } });
...
TEST_F(SampleInfoTest___get_gaps___Fixture,
    get_gaps___coverages_below_equal_and_above_threshold)
{
    sample_info_with_min_kmer_covg_10.set_coverage_information(
        { { 0, 0, 0 }, { 9, 10, 11, 9, 10, 9 } },
        { { 9, 10, 11 }, { 0, 0, 0, 0, 0, 0 } });
...
class SampleInfoTest___get_likelihoods_for_all_alleles___Fixture
    : public ::testing::Test {
public:
    class SampleInfoMock : public SampleInfo {
    public:
        ...
        MOCK_METHOD(
            uint32_t, get_min_coverage_threshold_for_this_sample, (), (const override));
        MOCK_METHOD(uint32_t,
            get_total_mean_coverage_over_all_alleles_given_a_minimum_threshold,
            (uint32_t minimum_threshold), (const override));
        MOCK_METHOD(uint32_t, get_total_mean_coverage_given_a_minimum_threshold,
            (uint32_t allele, uint32_t minimum_threshold), (const override));
        MOCK_METHOD(double, get_gaps, (uint32_t allele), (const override));
        MOCK_METHOD(double, compute_likelihood,
            (bool min_coverage_threshold_is_satisfied, double expected_depth_covg,
                double total_mean_coverage_of_allele_above_threshold,
                double total_mean_coverage_of_all_other_alleles_above_threshold,
                double error_rate, double gaps),
            (const override));
    };
...
TEST_F(SampleInfoTest___get_likelihoods_for_all_alleles___Fixture,
    get_likelihoods_for_all_alleles)
{
    EXPECT_CALL(sample_info, get_number_of_alleles).WillRepeatedly(Return(2));
    ...
    EXPECT_CALL(sample_info,
        get_total_mean_coverage_over_all_alleles_given_a_minimum_threshold(5))
        .Times(1)
        .WillOnce(Return(100));
    ...
    EXPECT_CALL(sample_info, get_gaps(0)).Times(1).WillOnce(Return(0.3));

    EXPECT_CALL(sample_info,
        compute_likelihood(false,
            default_genotyping_options.get_sample_index_to_exp_depth_covg()[0], 0, 100,
            DoubleEq(default_genotyping_options.get_error_rate()), DoubleEq(0.3)))
        .Times(1)
    ...
    EXPECT_CALL(sample_info, get_gaps(1)).Times(1).WillOnce(Return(0.5));
...
public:
    ...
    public:
        using SampleInfo::SampleInfo;
        MOCK_METHOD(
            std::vector<double>, get_likelihoods_for_all_alleles, (), (const override));
        MOCK_METHOD(uint32_t, get_mean_coverage_both_alleles, (uint32_t allele),
            (const override));
    };

    SampleInfoTest___get_confidence___Fixture()
        : sample_info(0, 2, &genotyping_options_get_confidence)
        , default_sample_info(0, 2, &default_genotyping_options)
    {
    }
...
TEST_F(SampleInfoTest___get_confidence___Fixture,
    get_confidence___not_enough_difference_in_covg)
{
    std::vector<double> likelihoods({ -10, -3, -5 });
    EXPECT_CALL(sample_info, get_likelihoods_for_all_alleles)
        .Times(1)
...
TEST_F(SampleInfoTest___get_confidence___Fixture,
    get_confidence___returns_valid_confidence)
{
    std::vector<double> likelihoods({ -10, -3, -5 });
    EXPECT_CALL(sample_info, get_likelihoods_for_all_alleles)
        .Times(1)
...
TEST_F(SampleInfoTest___get_genotype_from_coverage___Fixture,
    valid_confidence_but_below_threshold)
{
    EXPECT_CALL(sample_info, get_confidence)
        .Times(1)
...
TEST_F(SampleInfoTest___get_genotype_from_coverage___Fixture,
    valid_confidence_and_above_threshold)
{
    EXPECT_CALL(sample_info, get_confidence)
        .Times(1)
...
TEST_F(SampleInfoTest___Fixture, to_string___no_flags_set___expects_FatalRuntimeError)
{
    ASSERT_EXCEPTION(default_sample_info.to_string(false, false), FatalRuntimeError,
        "Error on stringifying VCF record sample info: incompatible genotyping "
        "options");
}
...
TEST_F(
    SampleInfoTest___Fixture, compute_likelihood___min_coverage_threshold_is_satisfied)
{
    ...
    double gaps = 0.02;
    double actual
        = default_sample_info.compute_likelihood(min_coverage_threshold_is_satisfied,
            expected_depth_covg, total_mean_coverage_of_allele_above_threshold,
    ...
    double expected = -expected_depth_covg
        ...
        - expected_depth_covg * gaps + log(1 - exp(-expected_depth_covg)) * (1 - gaps);
    EXPECT_DOUBLE_EQ(actual, expected);
}

TEST_F(SampleInfoTest___Fixture,
    compute_likelihood___min_coverage_threshold_is_not_satisfied)
{
...
class SampleIndexToSampleInfoTemplateAllVisible
    : public SampleIndexToSampleInfoTemplate<SAMPLE_TYPE> {
public:
    using SampleIndexToSampleInfoTemplate<SAMPLE_TYPE>::SampleIndexToSampleInfoTemplate;
    using SampleIndexToSampleInfoTemplate<
        SAMPLE_TYPE>::emplace_back_several_empty_sample_infos;
    using SampleIndexToSampleInfoTemplate<SAMPLE_TYPE>::
        set_number_of_alleles_and_resize_coverage_information_for_all_samples;
    using SampleIndexToSampleInfoTemplate<
        SAMPLE_TYPE>::merge_other_samples_infos_into_this;
};
...
public:
    ...
    public:
        ...
        SampleInfoMock(const SampleInfoMock& other)
            : SampleInfoMock(other.get_sample_index(), 2, other.genotyping_options)
        {
        }
    ...
    void SetUp() override
    {
        sample_index_to_sample_info.emplace_back_several_empty_sample_infos(
            2, 2, &default_genotyping_options);
    }

    void TearDown() override { }

    SampleIndexToSampleInfoTemplateAllVisible<SampleInfoMock>
        sample_index_to_sample_info;
};

TEST_F(SampleIndexToSampleInfoTemplate___Fixture,
    emplace_back_several_empty_sample_infos___same_number_of_alleles)
{
    sample_index_to_sample_info.emplace_back_several_empty_sample_infos(
        3, 2, &default_genotyping_options);
    ...
    for (size_t index = 0; index < sample_index_to_sample_info.size(); ++index) {
        ...
        EXPECT_EQ(empty_coverage_vector,
            sample_index_to_sample_info[index].get_allele_to_forward_coverages());
        EXPECT_EQ(empty_coverage_vector,
            sample_index_to_sample_info[index].get_allele_to_reverse_coverages());
        EXPECT_EQ(&default_genotyping_options,
            sample_index_to_sample_info[index].get_genotyping_options());
    }
}

TEST_F(SampleIndexToSampleInfoTemplate___Fixture,
    emplace_back_several_empty_sample_infos___different_number_of_alleles___all_samples_get_fixed)
{
    sample_index_to_sample_info.emplace_back_several_empty_sample_infos(
        2, 5, &default_genotyping_options);

    std::vector<std::vector<uint32_t>> empty_coverage_vector { { 0 }, { 0 }, { 0 },
        { 0 }, { 0 } };
...
TEST_F(SampleIndexToSampleInfoTemplate___Fixture,
    merge_other_samples_infos_into_this___successful_merge)
{
    SampleIndexToSampleInfoTemplateAllVisible<SampleInfoMock>
        another_sample_index_to_sample_info;
    another_sample_index_to_sample_info.emplace_back_several_empty_sample_infos(
    ...
    EXPECT_CALL(sample_index_to_sample_info[0],
        merge_other_sample_info_into_this(
            Property(&SampleInfoMock::get_sample_index, 0)))
        .Times(1);
    EXPECT_CALL(sample_index_to_sample_info[1],
        merge_other_sample_info_into_this(
            Property(&SampleInfoMock::get_sample_index, 1)))
        .Times(1);

    sample_index_to_sample_info.merge_other_samples_infos_into_this(
        another_sample_index_to_sample_info);
}
...
class SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture
    : public ::testing::Test {
public:
    class SampleInfoMock : public SampleInfo {
    public:
        ...
        MOCK_METHOD(double, get_likelihood_of_gt_from_coverages_compatible, (),
            (const override));
        MOCK_METHOD(void, set_gt_from_coverages_compatible,
            (const boost::optional<uint32_t>& gt), (override));
    };
    ...
    void SetUp() override
    {
        ...
        ON_CALL(sample_info_gt_0_with_likelihood_minus_10,
            is_gt_from_coverages_compatible_valid)
            .WillByDefault(Return(true));
        ON_CALL(sample_info_gt_0_with_likelihood_minus_5,
            is_gt_from_coverages_compatible_valid)
            .WillByDefault(Return(true));
        ON_CALL(sample_info_gt_3_with_likelihood_minus_10,
            is_gt_from_coverages_compatible_valid)
            .WillByDefault(Return(true));
        ON_CALL(sample_info_gt_3_with_likelihood_minus_5,
            is_gt_from_coverages_compatible_valid)
            .WillByDefault(Return(true));

        ON_CALL(
            sample_info_gt_0_with_likelihood_minus_10, get_gt_from_coverages_compatible)
            .WillByDefault(Return(0));
        ON_CALL(
            sample_info_gt_0_with_likelihood_minus_5, get_gt_from_coverages_compatible)
            .WillByDefault(Return(0));
        ON_CALL(
            sample_info_gt_3_with_likelihood_minus_10, get_gt_from_coverages_compatible)
            .WillByDefault(Return(3));
        ON_CALL(
            sample_info_gt_3_with_likelihood_minus_5, get_gt_from_coverages_compatible)
            .WillByDefault(Return(3));

        ON_CALL(sample_info_gt_0_with_likelihood_minus_10,
            get_likelihood_of_gt_from_coverages_compatible)
...
TEST_F(SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture,
    first_sample_info_has_invalid_gt)
{
    EXPECT_CALL(sample_info_invalid_gt, set_gt_from_coverages_compatible).Times(0);
    EXPECT_CALL(
        sample_info_gt_0_with_likelihood_minus_10, set_gt_from_coverages_compatible)
        .Times(0);
...
TEST_F(SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture,
    second_sample_info_has_invalid_gt)
{
    EXPECT_CALL(
        sample_info_gt_0_with_likelihood_minus_10, set_gt_from_coverages_compatible)
        .Times(0);
...
TEST_F(
    SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture, both_gts_are_to_ref)
{
    EXPECT_CALL(
        sample_info_gt_0_with_likelihood_minus_10, set_gt_from_coverages_compatible)
        .Times(0);
    EXPECT_CALL(
        sample_info_gt_0_with_likelihood_minus_5, set_gt_from_coverages_compatible)
        .Times(0);
...
TEST_F(SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture,
    first_gt_is_0_second_is_3_gt_0_higher_likelihood)
{
    EXPECT_CALL(
        sample_info_gt_0_with_likelihood_minus_5, set_gt_from_coverages_compatible)
        .Times(0);
    EXPECT_CALL(sample_info_gt_3_with_likelihood_minus_10,
        set_gt_from_coverages_compatible(boost::optional<uint32_t>(0)))
        .Times(1);
...
TEST_F(SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture,
    first_gt_is_0_second_is_3_gt_3_higher_likelihood)
{
    EXPECT_CALL(sample_info_gt_0_with_likelihood_minus_10,
        set_gt_from_coverages_compatible(boost::optional<uint32_t>(boost::none)))
        .Times(1);
    EXPECT_CALL(
        sample_info_gt_3_with_likelihood_minus_5, set_gt_from_coverages_compatible)
        .Times(0);
...
TEST_F(SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture,
    first_gt_is_3_second_is_0_gt_3_higher_likelihood)
{
    EXPECT_CALL(
        sample_info_gt_3_with_likelihood_minus_5, set_gt_from_coverages_compatible)
        .Times(0);
...
TEST_F(SampleInfoTest___solve_incompatible_gt_conflict_with___Fixture,
    first_gt_is_3_second_is_0_gt_0_higher_likelihood)
{
    EXPECT_CALL(sample_info_gt_3_with_likelihood_minus_10,
        set_gt_from_coverages_compatible(boost::optional<uint32_t>(0)))
        .Times(1);
...
// LIKELIHOOD TESTS
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// REASON COMMENTED OUT: THERE IS NO WAY TO TRY TO COMPUTE THE LIKELIHOOD WITHOUT HAVING
// A SAMPLE ANYMORE
...
//
TEST_F(SampleInfoTest___Fixture,
    get_likelihoods_for_all_alleles___gets_correct_likelihood_simple_case)
{
    default_sample_info.set_coverage_information({ { 1 }, { 2 } }, { { 1 }, { 2 } });
...
{
    EXPECT_CALL(sample_info, get_gaps(0)).WillRepeatedly(Return(0.5));

    EXPECT_CALL(sample_info, get_gaps(1)).WillRepeatedly(Return(0.8));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// REASON COMMENTED OUT: ALREADY TESTED, SEE
// TEST_F(SampleInfoTest___get_confidence___Fixture,
// get_confidence___not_enough_total_covg)
...
//}
//
// REASON COMMENTED OUT: THERE IS NO REASON FOR THIS TEST AS DIFFERENT DEPTHS IN
// DIFFERENT SAMPLES DO NOT AFFECT LIKELIHOOD COMPUTATION
...
// CONFIDENCE TESTS
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// REASON COMMENTED OUT: THERE IS NO WAY TO TRY TO COMPUTE THE CONFIDENCE WITHOUT HAVING
// INFO ANYMORE
...
TEST_F(SampleInfoTest___get_confidence___Fixture,
    get_confidence___gets_correct_confidence_simple_case)
{
    ...
    std::tie(index, confidence, max_likelihood)
        = *(default_sample_info.get_confidence());
    EXPECT_EQ(1, index);
...
TEST_F(SampleInfoTest___get_confidence___Fixture,
    get_confidence___gets_correct_confidence_two_alts)
{
    EXPECT_CALL(default_sample_info, get_likelihoods_for_all_alleles)
...
TEST_F(SampleInfoTest___get_confidence___Fixture, get_confidence___handles_ref_covg_0)
{
    EXPECT_CALL(default_sample_info, get_likelihoods_for_all_alleles)
        .WillRepeatedly(Return(
            std::vector<double>({ std::numeric_limits<double>::lowest(), -1.5 })));
    EXPECT_CALL(default_sample_info, get_mean_coverage_both_alleles)
        .WillRepeatedly(Return(0));
...
TEST_F(SampleInfoTest___get_confidence___Fixture, get_confidence___handles_alt_covg_0)
{
    EXPECT_CALL(default_sample_info, get_likelihoods_for_all_alleles)
        .WillRepeatedly(Return(
            std::vector<double>({ -1.5, std::numeric_limits<double>::lowest() })));
    EXPECT_CALL(default_sample_info, get_mean_coverage_both_alleles)
        .WillRepeatedly(Return(0));
...
// sample 0 missing confidence: done in
// TEST_F(SampleInfoTest___get_genotype_from_coverage___Fixture, invalid_confidence)
// sample 1 confidence below threshold: done in
// TEST_F(SampleInfoTest___get_genotype_from_coverage___Fixture,
// valid_confidence_but_below_threshold) sample 2 confidence above threshold, but has
// correct GT 0 already: done in
// TEST_F(SampleInfoTest___get_genotype_from_coverage___Fixture,
// valid_confidence_and_above_threshold) sample 3 confidence above threshold, but has
// correct GT 1 already: done in
// TEST_F(SampleInfoTest___get_genotype_from_coverage___Fixture,
// valid_confidence_and_above_threshold) sample 4 confidence above threshold, has
// incorrect GT 0: there is no way to have an incorrect GT from coverage: it is derived
// from the coverage sample 5 confidence above threshold, has incorrect GT 1: there is
// no way to have an incorrect GT from coverage: it is derived from the coverage
