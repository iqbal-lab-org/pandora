...
class PanRead {
  public:
    ...
    std::vector<std::unordered_map<uint32_t,std::set<MinimizerHit*, pComp_path>> hits; // from node id to cluster of hits against that node in this read

    PanRead(const uint32_t);
    void add_hits(const uint32_t, std::set<MinimizerHit*, pComp>&);
