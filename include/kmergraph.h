...
class KmerGraph {
  ...
  public:
    ...
    void get_next(const uint16_t, const uint8_t, const uint8_t, std::unordered_set<uint16_t>&, std::vector<std::deque<KmerNodePtr>>&);
    ...
    void find_all_compatible_paths(std::vector<std::deque<KmerNodePtr>>&, std::vector<std::vector<std::pair<uint16_t, uint16_t>>>&, const uint8_t covg_thresh=40,const uint8_t read_share_thresh=10);
