/*
 * Adapted from
 * https://github.com/iqbal-lab/cortex/blob/master/include/basic/binary_kmer.h
 *
...
public:
    /*
     * todo: I wonder if having the kmer size stored on the class is excess memory that
     * isn't necessary? We would just require a user to pass it in when needed. It
     * just makes some of the comparison operators a bit harder to implement if we
     * dont have this member variable.
     */
    ...
    // boolean binary_kmer_comparison_operator(const BinaryKmer const left, const BinaryKmer
    // const right);
    friend bool operator==(const BinaryKmer& left, const BinaryKmer& right);
    friend bool operator!=(const BinaryKmer& left, const BinaryKmer& right);

    // boolean binary_kmer_less_than(const BinaryKmer const left, const
    // BinaryKmer const right,short kmer_size);
    friend bool operator<=(const BinaryKmer& left, const BinaryKmer& right);
...
// int get_sliding_windows_from_sequence(char * sequence,char * qualities, int length,
// char quality_cutoff, short kmer_size, KmerSlidingWindowSet * windows, int max_windows,
// int max_kmers);
int get_sliding_windows_from_sequence(char* seq, char* qualities, int length,
    char quality_cut_off, short kmer_size, KmerSlidingWindowSet* windows,
...
BinaryKmer* binary_kmer_reverse_complement(
    BinaryKmer* kmer, short kmer_size, BinaryKmer* prealloc_reverse_kmer);
...
void binary_kmer_alloc_kmers_set(
