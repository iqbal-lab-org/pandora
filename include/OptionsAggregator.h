...
public:
    GenotypingOptions(const std::vector<uint32_t> &sampleIndexToExpDepthCovg, float errorRate, uint16_t confidenceThreshold,
                      uint32_t minAlleleCovg, float minFractionAlleleCovg, uint32_t minSiteTotalCovg,
                      uint32_t minSiteDiffCovg, uint32_t minKmerCovg, bool snpsOnly) : sample_index_to_exp_depth_covg(
            sampleIndexToExpDepthCovg), error_rate(errorRate), confidence_threshold(confidenceThreshold),
                                                                                       min_allele_covg(minAlleleCovg),
                                                                                       min_fraction_allele_covg(
                                                                                               minFractionAlleleCovg),
                                                                                       min_site_total_covg(
                                                                                               minSiteTotalCovg),
                                                                                       min_site_diff_covg(
                                                                                               minSiteDiffCovg),
    ...
    uint16_t get_confidence_threshold() const {
        return confidence_threshold;
    }

    uint32_t get_min_allele_covg() const {
    ...
    uint32_t get_min_site_total_covg() const {
        return min_site_total_covg;
    }

    uint32_t get_min_site_diff_covg() const {
        return min_site_diff_covg;
    }

    uint32_t get_min_kmer_covg() const {
        return min_kmer_covg;
    }
